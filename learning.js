...
async function createScoreCard(session) {
  ...
  context.fillText("華 ↔ 客｜華語客語詞典", 74, 100);
  context.font = '900 64px system-ui, -apple-system, "Segoe UI", sans-serif';
  context.fillText("客語詞語隨機挑戰", 74, 190);
  ...
  context.fillText(
    session.total === 10
      ? "每局完全隨機 10 題・官方客語發音"
      : `錯題隨機重考 ${session.total} 題・官方客語發音`,
    74,
    490,
  );
  context.font = '650 26px system-ui, -apple-system, "Segoe UI", sans-serif';
  context.fillText("yazelin.github.io/mandarin-hakka/", 74, 550);
...
export function initializeLearning({ dictionary, playAudio, sourceUrl = "https://hakkadict.moe.edu.tw/" }) {
  ...
  const accents = Array.isArray(dictionary?.metadata?.accents) ? dictionary.metadata.accents : [];
  const poolsByAccent = new Map(
    accents.map((accent) => [accent, buildQuizPool(dictionary, { accent })]),
  );
  const pool = [...new Map(
    [...poolsByAccent.values()].flat().map((candidate) => [candidate.id, candidate]),
  ).values()];
  ...
  function renderChallengeHome() {
    ...
    const quizPool = poolsByAccent.get(quizAccent) || [];
    const answerCount = new Set(quizPool.map((candidate) => candidate.answer)).size;
    card.append(
      makeElement("p", { className: "eyebrow", text: "聽聲音・揣意思" }),
      makeElement("h1", { text: "客語詞語隨機挑戰" }),
      makeElement("p", {
    ...
    if (accents.length) {
      const filter = makeElement("label", { className: "challenge-accent" });
      filter.append(document.createTextNode("挑戰腔調 "));
      const select = makeElement("select", { attributes: { "aria-label": "挑戰腔調" } });
      for (const accent of accents) {
        const option = new Option(accent, accent, false, accent === quizAccent);
        select.append(option);
      }
      select.addEventListener("change", () => {
        quizAccent = select.value;
        renderChallengeHome();
      });
    ...
    start.disabled = quizPool.length < 10;
    start.addEventListener("click", () => startChallenge(quizPool, 10));
    ...
    card.append(
      ...
      makeElement("p", {
        ...
        text: "不計時、不登入。答錯會自動留在這台裝置；連續答對兩次就視為掌握。挑戰題使用本站同來源音檔；可先在查詞頁選定腔調，下載該腔 360 題學習語音供離線練習。",
      }),
    );
    shell.append(card);
  ...
  function startChallenge(sourcePool = poolsByAccent.get(quizAccent) || [], count = 10) {
    if (sourcePool.length < count || sourcePool.length < 4) {
      renderChallengeHome();
      return;
    }
    const questions = buildQuizRound(sourcePool, count);
  ...
  function renderQuestion(answered = null) {
    ...
    prompt.append(makeElement("p", { className: "learning-muted", text: "聽這個客語詞，選出華語意思" }));
    const listen = makeButton(answered ? "再聽一次" : "▶ 聽客語", "quiz-audio-button");
    ...
    listen.addEventListener("click", async () => {
      ...
      audioStatus.textContent = played ? "可重播；聽完再選答案。" : "音檔無法播放，可用下方客語拼音提示作答。";
      listen.disabled = false;
    });
    prompt.append(listen, audioStatus);
    if (!answered) {
      const hint = makeButton("顯示客語拼音提示", "text-button");
      hint.addEventListener("click", () => {
        audioStatus.textContent = question.romanization;
        audioStatus.lang = "hak-Latn";
        hint.remove();
      });
      prompt.append(hint);
    }
  ...
  function answerFeedback(question, answered) {
    ...
    feedback.append(
      ...
      makeElement("p", {
        className: "quiz-feedback__pronunciation",
        text: question.romanization,
        attributes: { lang: "hak-Latn" },
      }),
      makeElement("p", {
        className: "learning-muted",
        text: question.accents.length ? `腔調：${question.accents.join("、")}` : "教育部官方詞條音檔",
      }),
    );
    feedback.append(
      makeElement("a", {
        text: "教育部臺灣客語辭典 ↗",
        attributes: { href: sourceUrl, target: "_blank", rel: "noopener noreferrer" },
      }),
    );
  ...
  async function shareScore(status) {
    if (!shareBlob) return;
    const text = `我在客語詞語隨機挑戰答對 ${session.score}/${session.total} 題，你也來試試！`;
    const url = `${window.location.origin}${window.location.pathname}#challenge`;
    try {
      if (typeof File === "function" && navigator.share && navigator.canShare) {
        const file = new File([shareBlob], "hakka-challenge-score.png", { type: "image/png" });
        if (navigator.canShare({ files: [file] })) {
          await navigator.share({ title: "客語詞語隨機挑戰", text, url, files: [file] });
          status.textContent = "分享完成。";
          return;
        }
      }
      downloadBlob(shareBlob, "hakka-challenge-score.png");
      let copied = false;
      try {
  ...
  function renderWrongbook() {
    ...
    exportButton.addEventListener("click", () => {
      ...
      downloadBlob(blob, "mandarin-hakka-learning.json");
    });
  ...
  function renderFlashcard(revealed) {
    ...
    if (revealed) {
      face.append(
        ...
        makeElement("span", {
          className: "flashcard__pronunciation",
          text: candidate.romanization,
          attributes: { lang: "hak-Latn" },
        }),
      ...
      const listen = makeButton("▶ 聽客語", "quiz-audio-button");
