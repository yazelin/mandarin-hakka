...
test("documented Hakka delivery identity matches the runtime constants", () => {
  ...
  const canonicalUrls = ["CORE_DATA_URL", "DETAILS_DATA_URL"].map(
  ...
  assert.match(primaryBase, /^https:\/\/cdn\.jsdelivr\.net\/gh\/yazelin\/mandarin-hakka@[0-9a-f]{40}\/$/);
...
test("documented Hakka payload hashes and CDN limits match deployed files", () => {
  const app = read("app.js");
  const contract = read("docs/DATA-RELEASE.md");
  const pin = app.match(/const PRIMARY_DATA_BASE = "[^"@]+@([0-9a-f]{40})\//)?.[1] || "";
  for (const path of ["data/dictionary-core.json", "data/dictionary-details.json"]) {
  ...
  const tracked = execFileSync("git", ["ls-files", "-z"], { cwd: root })
    ...
    .split("\0")
    .filter(Boolean);
  const packageBytes = tracked.reduce((total, path) => total + statSync(resolve(root, path)).size, 0);
  assert.ok(packageBytes < 150_000_000, `tracked package is ${packageBytes} bytes`);
});
